...
-type(amqp_method_field_name() ::
       ( out_of_band | nowait | queue | consumer_tag
       | reply_text | multiple | routing_key | ticket
       | passive | destination | nowait | ticket
       | heartbeat | durable | ticket | ticket
       | message_count | message_count | nowait | reply_code
       | known_hosts | immediate | active | consumer_tag
       | realm | exchange | ticket | internal
       | source | version_minor | exchange | queue
       | frame_max | exchange | nowait | ticket
       | routing_key | exchange | global | exclusive
       | arguments | requeue | auto_delete | consumer_tag
       | server_properties | arguments | destination | routing_key
       | response | routing_key | arguments | if_unused
       | version_major | nowait | delivery_tag | delivery_tag
       | mechanism | channel_max | passive | consumer_tag
       | class_id | client_properties | insist | prefetch_size
       | prefetch_count | reason | nowait | nowait
       | ticket | exchange | nowait | ticket
       | nowait | auto_delete | mandatory | credit_drained
       | channel_max | consumer_count | response | active
       | message_count | method_id | ticket | ticket
       | routing_key | consumer_tag | delivery_tag | routing_key
       | redelivered | requeue | multiple | reply_text
       | arguments | cluster_id | virtual_host | source
       | frame_max | if_empty | exchange | reply_text
       | no_ack | write | consumer_tag | queue
       | routing_key | active | consumer_tag | channel_id
       | delivery_tag | queue | credit | nowait
       | locales | class_id | exclusive | queue
       | capabilities | queue | ticket | ticket
       | heartbeat | arguments | redelivered | type
       | reply_code | delivery_tag | exchange | read
       | reply_code | requeue | no_local | passive
       | arguments | arguments | mechanisms | message_count
       | exclusive | method_id | nowait | no_ack
       | ticket | if_unused | requeue | available
       | drain | challenge | locale | exchange
       | queue | routing_key | durable | queue )).
