...
-type(amqp_method_field_name() ::
       ...
       | host | integer_3 | ticket | no_local
       | routing_key | frame_max | reply_text | routing_key
       | out_of_band | consumer_tag | routing_key | durable
       | method_id | exchange | nowait | queue
       | routing_key | nowait | exchange | result
       | ticket | routing_key | challenge | integer_4
       | locales | locale | content_size | content_checksum
       | exchange | delivery_tag | queue | consumer_tag
       | operation | consume_rate | reply_text | consumer_tag
       | ticket | consumer_count | nowait | mandatory
       | nowait | exclusive | nowait | redelivered
       | operation | exchange | message_count | auto_delete
       | channel_max | global | consumer_tag | message_count
       | method_id | delivery_tag | delivery_tag | ticket
       | mandatory | if_empty | exchange | reply_code
       | consumer_tag | nowait | prefetch_size | details
       | passive | nowait | ticket | no_local
       | meta_data | class_id | reply_code | ticket
       | consumer_tag | insist | known_hosts | routing_key
       | frame_max | reply_text | reply_code | response
       | global | exclusive | delivery_tag | read
       | no_ack | reply_code | consumer_tag | exchange
       | version_major | message_count | integer_result | identifier
       | requeue | string_1 | exclusive | active
       | exclusive | string_op | staged_size | nowait
       | immediate | multiple | prefetch_count | type
       | ticket | routing_key | result | heartbeat
       | arguments | ticket | passive | reply_code
       | integer_1 | consumer_tag | mechanism | routing_key
       | table | delivery_tag | arguments | queue
       | version_minor | identifier | identifier | reply_code
       | client_properties | ticket | auto_delete | reply_text
       | ticket | consumer_tag | queue | arguments
       | realm | ticket | reply_text | routing_key
       | ticket | queue | redelivered | delivery_tag
       | channel_max | redelivered | requeue | ticket
       | nowait | nowait | exchange | server_properties
       | routing_key | consumer_tag | prefetch_size | immediate
       | queue | reply_text | arguments | exchange
       | active | write | no_ack | integer_2
       | no_local | nowait | no_ack | mandatory
       | integer_op | queue | if_unused | requeue
       | prefetch_size | queue | exchange | global
       | string_result | dtx_identifier | queue | ticket
       | exchange | nowait | exchange | if_unused
       | known_hosts | queue | consumer_tag | response
       | mechanisms | ticket | exchange | class_id
       | heartbeat | exchange | exchange | multiple
       | queue | active | consumer_tag | passive
       | durable | capabilities | virtual_host | routing_key
       | delivery_tag | prefetch_count | internal | consumer_tag
       | requeue | consumer_tag )).
